...
if (isset($_GET['duplicate'], $_GET['token']) && !FM_READONLY) {
  ...
  <form action="" method="post" class="pt-3">
      ...
      <div class="table-responsive" style="overflow-x: auto; flex: 1;">
          <table class="table table-bordered table-hover table-sm" id="main-table" data-bs-theme="<?php echo FM_THEME; ?>" style="margin-bottom: 50px;">
  ...
  <div class="pane-resizer" id="details-resizer"></div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function initResizer(resizerId, paneId, edge) {
        ...
        function doDrag(e) {
            let newWidth = edge === 'right' ? startWidth + e.clientX - startX : startWidth - (e.clientX - startX);
    ...
    initResizer('sidebar-resizer', 'sidebar-pane', 'right');
    initResizer('details-resizer', 'details-pane', 'left');

    // 4. Keyboard Navigation
    document.addEventListener('keydown', function(e) {
        if (e.target.closest('input, textarea, select, [contenteditable="true"]')) return;
        
        const selected = document.querySelector('.explorer-selected');
        const itemsList = Array.from(document.querySelectorAll('tr[data-name], .grid-item[data-name]'));
        if (!itemsList.length) return;

        let index = selected ? itemsList.indexOf(selected) : -1;

        if (e.key === 'ArrowDown' || e.key === 'ArrowRight') {
            e.preventDefault();
            index = (index + 1) % itemsList.length;
            itemsList[index].click();
            itemsList[index].scrollIntoView({ block: 'nearest' });
        } else if (e.key === 'ArrowUp' || e.key === 'ArrowLeft') {
            e.preventDefault();
            index = index <= 0 ? itemsList.length - 1 : index - 1;
            itemsList[index].click();
            itemsList[index].scrollIntoView({ block: 'nearest' });
            ...
            e.preventDefault();
            document.getElementById('det-delete-btn').click();
        }
    });
    ...
    let isMarquee = false, startM = {x:0, y:0};

    mainPane.addEventListener('mousedown', function(e) {
        if (e.target !== mainPane && !e.target.classList.contains('table-responsive')) return;
        if (e.button !== 0) return; // Left click only
        
        isMarquee = true;
        const rect = mainPane.getBoundingClientRect();
        startM = { x: e.clientX - rect.left, y: e.clientY - rect.top + mainPane.scrollTop };
        
        marquee.style.left = startM.x + 'px';
        marquee.style.top = startM.y + 'px';
    ...
    document.addEventListener('mousemove', function(e) {
        if (!isMarquee) return;
        const rect = mainPane.getBoundingClientRect();
        const cur = { x: e.clientX - rect.left, y: e.clientY - rect.top + mainPane.scrollTop };
        ...
        const allItems = document.querySelectorAll('tr[data-name], .grid-item[data-name]');
        allItems.forEach(item => {
            const iRect = item.getBoundingClientRect();
            const intersect = !(mRect.right < iRect.left || mRect.left > iRect.right || mRect.bottom < iRect.top || mRect.top > iRect.bottom);
...
.explorer-selected { background: rgba(13, 110, 253, 0.1) !important; outline: 1px solid var(--accent); }
  ...
  <style>
      ...
      #explorer-layout {
          display: flex;
          height: calc(100vh - 56px); /* Adjusted to standard navbar height */
          overflow: hidden;
          margin: 0;
          padding: 0;
      }
